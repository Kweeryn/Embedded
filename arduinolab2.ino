...
const unsigned short int GREEN_LED = 7;
const unsigned short int YELLOW_LED = 6;
const unsigned short int RED_LED = 5; 
const unsigned short int POTENTIOMETER = A0;
...
void setup() {
  ...
  Timer1.attachInterrupt(blink_interrupt);  
}

void blink_interrupt() {
  T = millis() / double(1000);
  k = analogRead(POTENTIOMETER) / int(100);
  
  analogWrite(GREEN_LED, 255 * (sin(k * T) + 1) / 2);
  analogWrite(YELLOW_LED, 255 * (sin(k * T + 2 * pi / 3) + 1)/ 2);
  analogWrite(RED_LED, 255 * (sin(k * T + 4 * pi / 3) + 1) / 2);
}
